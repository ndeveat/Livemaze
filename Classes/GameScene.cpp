...
bool GameScene::init()
{
	...
	// ------------------------------------------------------------[ Tiled Map Setting ]
	tiled_map = TMXTiledMap::create("Floor/GameStage/GameStage1.tmx");
	tiled_map->setAnchorPoint(Vec2(0.0f, 1.0f));
	tiled_map->setPosition(Vec2(0.0f, HEIGHT * 4));
	tiled_map->setTag(4);
	tiled_map->setName("TILE_MAP");
	...
	for (int i = 0; i < 6; i++){
		...
		bugTest->setPosition(Vec2(3116 + cocos2d::random(1, 20), 486 + cocos2d::random(1, 20)));
	...
	for (int i = 0; i < 4; i++){
		...
		catTest->setPosition(Vec2(4150 + cocos2d::random(1, 35), 1441 + cocos2d::random(1, 35)));
	...
	for (int i = 0; i < 6; i++)
	{
		...
		snake->setPosition(Vec2(2271 + random(1, 100), 459 + random(1, 50)));
	...
	{
		auto snake = Snake::createEnemy("Enemy/Snake/snake.plist", player, unitLayer);
		snake->setPosition(Vec2(2571 + random(1, 100), 659 + random(1, 50)));
		snake->setTMX(tiled_map, tiled_layer_metainfo);
		tiled_map->addChild(snake);
	}
	...
	{
		auto snake = Snake::createEnemy("Enemy/Snake/snake.plist", player, unitLayer);
		snake->setPosition(Vec2(4201.86377 + random(1, 100), 1041.47046 + random(1, 50)));
		snake->setTMX(tiled_map, tiled_layer_metainfo);
		tiled_map->addChild(snake);
	}
	...
	for (int i = 0; i < 10; i++)
	{
		...
		mummy->setPosition(Vec2(3065.24609 + random(1, 100), 1119.71716 + random(1, 100)));
		mummy->setTMX(tiled_map, tiled_layer_metainfo);
		tiled_map->addChild(mummy);
	}

	auto trapItem = Item::TRAP(Vec2(3065.24609, 1119.71716));
	tiled_map->addChild(trapItem);
	item_vector.push_back(trapItem);


	for (int i = 0; i < 6; i++){
		auto bugTest = Bug::createEnemy("Enemy/Bug/bug.plist", player, unitLayer);
		bugTest->setPosition(Vec2(2554.87598 + cocos2d::random(1, 20), 2131.19507 + cocos2d::random(1, 20)));
		bugTest->setTMX(tiled_map, tiled_layer_metainfo);
		bugTest->back_pos = bugTest->getPosition();
		tiled_map->addChild(bugTest, 1);
	}
	...
	{
		auto mummy = Mummy::createEnemy("Enemy/Mummy/mummy.plist", player, unitLayer);
		mummy->setPosition(Vec2(463.772034 + random(1, 100), 1301.16187 + random(1, 100)));
		mummy->setTMX(tiled_map, tiled_layer_metainfo);
		tiled_map->addChild(mummy);
	}

	auto trapItem2 = Item::TRAP(Vec2(285.621948, 1301.16187));
	tiled_map->addChild(trapItem2);
	item_vector.push_back(trapItem2);


	auto trapItem3 = Item::TRAP(Vec2(3832.87500, 1259.99097));
	tiled_map->addChild(trapItem3);
	item_vector.push_back(trapItem3);


	auto trapItem4 = Item::TRAP(Vec2(1380.46228, 1291.83154));
	...
	for (int i = 0; i < 6; i++){
		auto bugTest = Bug::createEnemy("Enemy/Bug/bug.plist", player, unitLayer);
		bugTest->setPosition(Vec2(2005.45776 + cocos2d::random(1, 20), 1145.87988 + cocos2d::random(1, 20)));
		bugTest->setTMX(tiled_map, tiled_layer_metainfo);
		bugTest->back_pos = bugTest->getPosition();
		tiled_map->addChild(bugTest, 1);
	}
	...
	//	snake->setPosition(Vec2(471.818207 + random(1, 100), 1308.98792 + random(1, 5)));
	//	snake->setTMX(tiled_map, tiled_layer_metainfo);
	//	tiled_map->addChild(snake);
	//}
	...
	//	snake->setPosition(Vec2(3077.21265 + random(1, 100), 1114.55420 + random(1, 5)));
	//	snake->setTMX(tiled_map, tiled_layer_metainfo);
	//	tiled_map->addChild(snake);
	//}

	auto sphinx = Sphinx::createEnemy("Enemy/Sphinx/sphinx.plist", player, interfaceLayer);
	sphinx->setPosition(Vec2(2554.12964, 1580.40430));
	sphinx->setZOrder(2);
	sphinx->setName("Sphinx");
	sphinx->setTag(91);
	tiled_map->addChild(sphinx);
...
bool GameScene::NextPosition(Node* target)
{
...
bool GameScene::EnemyNextPosition(Vec2 position)
{
...
bool GameScene::NextPosition(Vec2 position)
{
	Vec2 pos = GetTileCoordinateWithNodeSpacePosition(position);
	if (pos == Vec2(-1, -1))
		return false;
...
Vec2 GameScene::GetTileCoordinateWithNodeSpacePosition(Vec2 position)
{
...
bool GameScene::onTouchBegan(Touch *touch, Event *unused_even)
{
...
void GameScene::ArraySaveTileMap()
{
...
void GameScene::CheateKeyUpdate()
{
...
void GameScene::Player_Item_Update()
{
	// ----------------------- [ 아이템 업데이트 ]
	if (item_vector.size())
	{
		...
		for (auto trap : attack_item_vector)
		{
			...
			if (trap->getBoundingBox().intersectsRect(player_bounding))
			{
				if (trap->getTag() == 1)
				{
					...
					itemLayer->removeChild(trap);
					tiled_map->removeChild(trap);
					attack_item_vector.erase(find(attack_item_vector.begin(), attack_item_vector.end(), trap));
					return;
				}
			}
		}
...
void GameScene::Enemy_AttackItem_Update()
{
	// ----------------------- [ 공격형 아이템을 밟았을때 ]
	if (attack_item_vector.size()){
...
void GameScene::InterfaceUpdate()
{
	// 플레이어 업데이트
...
void GameScene::EnemyUpdate(float delta)
{
